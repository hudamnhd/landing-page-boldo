  ...
  return (
    <section>
      {servicesGroupC.map((content) => (
        <div
          key={content.id}
          className="my-12 space-y-5 px-10 drop-shadow-md sm:container sm:mx-auto sm:my-36 sm:space-y-10"
        >
          ...
          <div className="flex flex-col justify-between space-y-7 md:flex-row md:space-y-0 ">
            ...
            <div className="flex flex-col ">
              {content.content.map((subdata) => (
                <p
                  key={subdata.sub}
                  className="text-md mb-4 flex items-center justify-between border-b border-black pb-4 md:text-lg"
                >
                  {subdata.sub}
                  <span>
